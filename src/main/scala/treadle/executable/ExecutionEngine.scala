...
//    val symbolsToDo: Seq[Symbol] = {
//      symbolTable.registerNames.map(name => symbolTable(name)).toSeq ++
//        symbolTable.moduleMemoryToMemorySymbol.values.flatten
//    }
    val symbolsToDo: Seq[Symbol] = symbolTable.symbols.toSeq

    symbolsToDo.foreach { symbol =>
      rand.setSeed(symbol.name.hashCode + userRandomSeed)
